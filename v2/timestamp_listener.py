    ...
    while True:
        ...
        if msg_type == "CAPTURE":
            ...
            if frame_id in frame_dict:
                # print(f"Received SEND message for frame {frame_id} with matching CAPTURE message")
                ...
                if ros_to_gst_deltas and msg_count % 10 == 0:
                    avg_delta = sum(ros_to_gst_deltas) / len(ros_to_gst_deltas)
                    print(f"Average time from ROS capture to GStreamer send: {avg_delta:.3f} seconds over {len(ros_to_gst_deltas)} frames")
                if len(ros_to_gst_deltas) > 10:
            ...
            if frame_id in frame_dict:
                ...
                bandwidth = total_bytes_received / elapsed_time if elapsed_time > 0 else 0

                if msg_count % 100 == 0:
                    if hostgst_to_clientgst_deltas:
                        avg_delta = sum(hostgst_to_clientgst_deltas) / len(hostgst_to_clientgst_deltas)
                        print(f"Average time from GStreamer send to recv receive: {avg_delta:.3f} seconds over {len(hostgst_to_clientgst_deltas)} frames")
                    if len(hostgst_to_clientgst_deltas) > 10:
                        hostgst_to_clientgst_deltas.popleft()  
                    print(f"Total bytes received: {total_bytes_received}, Elapsed time: {elapsed_time:.2f} seconds, Average bandwidth: {bandwidth:.2f} bytes/sec")
            else:
            ...
            # print("display: ", rest)
            if frame_id in frame_dict:
                # print(f"Received DISPLAY message for frame {frame_id} with matching SEND message")
                sent_pts, sent_duration, sent_localtime, sent_length, last_msg_type = frame_dict[frame_id]
                ...
                if clientgst_to_display_deltas and msg_count % 100 == 0:
                    avg_delta = sum(clientgst_to_display_deltas) / len(clientgst_to_display_deltas)
                    print(f"Average time from GStreamer send to display receive: {avg_delta:.3f} seconds over {len(clientgst_to_display_deltas)} frames")
                if len(clientgst_to_display_deltas) > 10:
