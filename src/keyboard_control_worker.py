...
class KeyboardControlState:
    ...
    def to_command(self, sequence: int) -> VehicleControlCommand:
...
class KeyboardControlWorker:
    def __init__(self, args: argparse.Namespace) -> None:
        self._bind = args.bind
        self._topic = args.topic
        self._rate_hz = args.rate_hz
        self._verbose = args.verbose
        self._state = KeyboardControlState(axis_increment=args.axis_increment)
    ...
    def run(self) -> None:
        ...
        print(f"Publishing keyboard commands on {self._bind} topic={self._topic!r} at {self._rate_hz:.1f} Hz")
        ...
        try:
            ...
            self._socket.close()
            self._context.term()
            print("\nKeyboard control worker stopped.")
    ...
    def _publish_next_command(self) -> None:
        command = self._state.to_command(self._sequence)
...
def main() -> None:
    KeyboardControlWorker(build_parser().parse_args()).run()
